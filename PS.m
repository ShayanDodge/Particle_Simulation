...
for k=1:4^(n)
  ...
  cells(k).phi_z_i=zeros(N_cell(k),1);% the direct term at each particles location
...
phi(:,:,n+1) = Q_cell(:,:,n+1).*log10(z(:,:,n+1))+(a_1(:,:,n+1)./z(:,:,n+1));
%% ==============step 2====================================================
    ...
    for i=1:(sqrt(4^(k-1)))
        for j=1:(sqrt(4^(k-1)))
            for ci=(i-1).*sqrt(4^(n-(k-1)))+1:(i).*sqrt(4^(n-(k-1)))
                ...
                phi(i,j,k)=phi(i,j,k)+...
                Q_cell(ci,cj,k+1).*log10(z(i,j,k))-Q_cell(ci,cj,k+1).*...
                z(ci,cj,k+1)./z(i,j,k);
                end
            end
        end
...
psi(i,j,k)=psi(i,j,k)+...
-a_1(ci,cj,k)./z(ci,cj,k)+Q_cell(ci,cj,k).*log10(-z(ci,cj,k))+...
